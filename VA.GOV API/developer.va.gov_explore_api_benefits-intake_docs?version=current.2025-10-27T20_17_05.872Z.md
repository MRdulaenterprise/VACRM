[Skip to main content](https://developer.va.gov/explore/api/benefits-intake/docs?version=current#main)
...
3. [Benefits Intake API](https://developer.va.gov/explore/api/benefits-intake)
4. [Docs](https://developer.va.gov/explore/api/benefits-intake/docs?version=current#content)

[Skip Page Navigation](https://developer.va.gov/explore/api/benefits-intake/docs#page-header) In this section

- [Benefits Intake API](https://developer.va.gov/explore/api/benefits-intake)
- [Docs](https://developer.va.gov/explore/api/benefits-intake/docs)
- [Test data](https://developer.va.gov/explore/api/benefits-intake/test-users)
- [Release notes](https://developer.va.gov/explore/api/benefits-intake/release-notes)
- [Sandbox access](https://developer.va.gov/explore/api/benefits-intake/sandbox-access)

# Docs

## Benefits Intake API

[https://api.va.gov/internal/docs/benefits-intake/v1/openapi.json](https://api.va.gov/internal/docs/benefits-intake/v1/openapi.json)
...
| --- | --- |
| **Pending** | \- This is the initial status. Indicates no document submission has been uploaded yet.<br>\- Date of Receipt is not yet established with this status. |
| **Uploaded** | \- Indicates document submission has been successfully uploaded (PUT) to the API server.<br>\- Submission has not yet been validated.<br>\- Date of Receipt is not yet established with this status.<br>\- Any errors with the document submission, such as having an unreadable PDF, may cause an Error status. |
| **Received** | \- Indicates document submission has been received downstream of the API and is awaiting processing.<br>\- The VA Date of Receipt is set when this status is achieved.<br>\- Any errors with the document submission, such as having an unreadable PDF, may cause an Error status.<br>\- This is the final status in the sandbox environment unless further progress is simulated by the user. |
| **Processing** | \- Indicates the document package is being validated, processed, and made ready to route and work.<br>\- Any errors with the document submission, such as having an unreadable PDF, may cause an Error status. |
| **Success** | \- Indicates the document submission has been successfully received within VA's mail handling system.<br>\- Success is the final status for a small percentage of submissions with claim types, Veteran types, or exception processes that are not worked in VBMS. A true value in the final\_status attribute will indicate this.<br>\- Most submissions reach a Success status within 1 business day.<br>\- A small portion will take longer. However, some submissions may take up to 2 weeks to reach a Success status. |
...
| **Expired** | \- After a POST request, there is a 15-minute window during which documents must be uploaded via a PUT request.<br>\- An Expired status means the documents were not successfully uploaded within this 15-minute window.<br>\- We recommend coding to retry unsuccessful uploads within 15 minutes using the same submission in case of connection issues. |
...
- The POST /uploads/validate\_document endpoint can be used to ensure individual PDF documents meet system requirements prior to submission.
...
1. Client Request: POST [https://sandbox-api.va.gov/services/vba\_documents/v1/](https://sandbox-api.va.gov/services/vba_documents/v1/)
...
5. Client Request: GET [https://sandbox-api.va.gov/services/vba\_documents/v2/uploads/{id}](https://sandbox-api.va.gov/services/vba_documents/v2/uploads/%7Bid%7D)
...
5. Client Request: POST [https://sandbox-api.va.gov/services/vba\_documents/v1/uploads/report](https://sandbox-api.va.gov/services/vba_documents/v1/uploads/report)
...
[Back to topBack to top](https://developer.va.gov/explore/api/benefits-intake/docs?version=current#ds-back-to-top)
