[Skip to main content](https://developer.va.gov/explore/api/decision-reviews/docs?version=current#main)
...
3. [Decision Reviews API](https://developer.va.gov/explore/api/decision-reviews)
4. [Docs](https://developer.va.gov/explore/api/decision-reviews/docs?version=current#content)

[Skip Page Navigation](https://developer.va.gov/explore/api/decision-reviews/docs#page-header) In this section

- [Decision Reviews API](https://developer.va.gov/explore/api/decision-reviews)
- [Docs](https://developer.va.gov/explore/api/decision-reviews/docs)
- [Test data](https://developer.va.gov/explore/api/decision-reviews/test-users)
- [Release notes](https://developer.va.gov/explore/api/decision-reviews/release-notes)

# Docs

## Decision Reviews API

[https://api.va.gov/internal/docs/appeals-decision-reviews/v2/openapi.json](https://api.va.gov/internal/docs/appeals-decision-reviews/v2/openapi.json)

The Decision Reviews API allows you to interact with a Veteran's decision reviews, also known as benefit appeals. This API provides a secure and efficient alternative to paper or fax submissions and follows the AMA process. To view the status of all decision reviews and benefits appeals submitted according to the legacy benefits appeals process, use the [Appeals Status API](https://developer.va.gov/explore/appeals/docs/appeals?version=current).
...
Our sandbox environment is populated with [claimant test data](https://developer.va.gov/explore/api/decision-reviews/test-users) that can be used to test various response scenarios. This sandbox data contains no PII or PHI, but mimics real claimant account information.
...
| submitted | A submitted status means the data was successfully transferred to the central mail portal.<br>A submitted status is confirmation from the central mail portal that they have received the PDF, but the data is not yet being processed. The Date of Receipt is set when this status is achieved.<br>Submitted is the final status in the sandbox environment. |
...
The updated\_at field indicates the last time the status for a given GUID was updated.

### Higher-Level Reviews

POST
/higher\_level\_reviews

Creates a new Higher-Level Review

GET
/higher\_level\_reviews/{uuid}

Shows a specific Higher-Level Review. (a.k.a. the Show endpoint)

GET
/higher\_level\_reviews/contestable\_issues/{benefit\_type}

Returns all contestable issues for a specific veteran.

GET
/higher\_level\_reviews/schema

Gets the Higher-Level Review JSON Schema.

POST
/higher\_level\_reviews/validate
...
POST
/notice\_of\_disagreements

Creates a new Notice of Disagreement

GET
/notice\_of\_disagreements/{uuid}

Shows a specific Notice of Disagreement. (a.k.a. the Show endpoint)

GET
/notice\_of\_disagreements/schema

Gets the Notice of Disagreement JSON Schema.

POST
/notice\_of\_disagreements/validate

Validates a POST request body against the JSON schema.

POST
/notice\_of\_disagreements/evidence\_submissions
...
GET
/notice\_of\_disagreements/evidence\_submissions/{uuid}
...
POST
/supplemental\_claims

Creates a new Supplemental Claim

GET
/supplemental\_claims/{uuid}

Shows a specific Supplemental Claim. (a.k.a. the Show endpoint)

GET
/supplemental\_claims/schema

Gets the Supplemental Claims JSON Schema.

POST
/supplemental\_claims/validate

Validates a POST request body against the JSON schema.

POST
/supplemental\_claims/evidence\_submissions
...
Accepts Supplemental Claim Evidence Submission document upload.

GET
/supplemental\_claims/evidence\_submissions/{uuid}
...
GET
/contestable\_issues/{decision\_review\_type}
...
[Back to topBack to top](https://developer.va.gov/explore/api/decision-reviews/docs?version=current#ds-back-to-top)
